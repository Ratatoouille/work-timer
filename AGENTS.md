...
go run .                # Run app; opens DD_MM_YY.json for today (auto-created name)
...
- `default_file` — File to open on startup when no CLI arg given (default `""`). Takes priority over the auto-generated `DD_MM_YY.json` name.
- `language` — `"ru"` or `"en"`.
...
JSON in `work_dir`, one per day. Default name `DD_MM_YY.json` (e.g. `04_08_26.json`) when no `default_file` and no CLI arg.
...
8. **Autogenerated filename**: Without CLI args and without `default_file`, the app opens `<work_dir>/DD_MM_YY.json` (today's day/month/year). The file need not exist — it's created on save. Sorting in the file list uses the parsed date (day/month/year from the name) so that chronology stays correct across year changes.
